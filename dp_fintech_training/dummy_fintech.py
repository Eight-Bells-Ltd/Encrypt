...
def fit_model_dp(X_train, y_train):
    rf = RFDP(epsilon = 0.05, n_estimators = 100).fit(X_train, y_train)
    return rf

def return_accuracy(rf, X_test, y_test):
...
def write_accuracy_to_file(rf, X_test, y_test):
    """
    Evaluates the trained random forest model on a test set and writes the accuracy to a file.
    ...
    y_pred = rf.predict(X_test)
    accuracy = accuracy_score(y_test, y_pred)
    logger.info(f"Training complete. The accuracy of the model was: {accuracy:.4f}")
